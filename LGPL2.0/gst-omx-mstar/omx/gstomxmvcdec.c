...
GST_DEBUG_CATEGORY_STATIC (gst_omx_mvc_dec_debug_category);
#define GST_CAT_DEFAULT gst_omx_mvc_dec_debug_category

/* prototypes */
static gboolean gst_omx_mvc_dec_is_format_change (GstOMXVideoDec * dec,
    GstOMXPort * port, GstVideoCodecState * state);
static gboolean gst_omx_mvc_dec_set_format (GstOMXVideoDec * dec,
    GstOMXPort * port, GstVideoCodecState * state);
...
#define DEBUG_INIT \
  GST_DEBUG_CATEGORY_INIT (gst_omx_mvc_dec_debug_category, "omxmvcdec", 0, \
      "debug category for gst-omx video decoder base class");

G_DEFINE_TYPE_WITH_CODE (GstOMXMVCDec, gst_omx_mvc_dec,
    GST_TYPE_OMX_VIDEO_DEC, DEBUG_INIT);

static void
gst_omx_mvc_dec_class_init (GstOMXMVCDecClass * klass)
{
  GstOMXVideoDecClass *videodec_class = GST_OMX_VIDEO_DEC_CLASS (klass);
  GstElementClass *element_class = GST_ELEMENT_CLASS (klass);

  videodec_class->is_format_change =
      GST_DEBUG_FUNCPTR (gst_omx_mvc_dec_is_format_change);
  videodec_class->set_format = GST_DEBUG_FUNCPTR (gst_omx_mvc_dec_set_format);

  videodec_class->cdata.default_sink_template_caps = "video/x-h264, "
      "parsed=(boolean) true, "
      "stream-format=(string) { avc, avc3, byte-stream }, "
      "format=(string) mvc1, " "alignment=(string) { au, nal }";

  gst_element_class_set_static_metadata (element_class,
      "OpenMAX MVC Video Decoder",
      "Codec/Decoder/Video",
      "Decode MVC video streams", "MStar Semiconductor Inc");

  gst_omx_set_default_role (&videodec_class->cdata, "video_decoder.mvc");
}

static void
gst_omx_mvc_dec_init (GstOMXMVCDec * self)
{
}

static gboolean
gst_omx_mvc_dec_is_format_change (GstOMXVideoDec * dec,
    GstOMXPort * port, GstVideoCodecState * state)
{
  return FALSE;
}
...
static gboolean
gst_omx_mvc_dec_set_format (GstOMXVideoDec * dec, GstOMXPort * port,
    GstVideoCodecState * state)
{
  ...
  port_def.format.video.eCompressionFormat = OMX_VIDEO_CodingMVC;
