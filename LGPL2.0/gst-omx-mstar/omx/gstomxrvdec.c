...
GST_DEBUG_CATEGORY_STATIC (gst_omx_rv_dec_debug_category);
#define GST_CAT_DEFAULT gst_omx_rv_dec_debug_category

/* prototypes */
static gboolean gst_omx_rv_dec_is_format_change (GstOMXVideoDec * dec,
    GstOMXPort * port, GstVideoCodecState * state);
static gboolean gst_omx_rv_dec_set_format (GstOMXVideoDec * dec,
    GstOMXPort * port, GstVideoCodecState * state);
...
#define DEBUG_INIT \
  GST_DEBUG_CATEGORY_INIT (gst_omx_rv_dec_debug_category, "omxrvdec", 0, \
      "debug category for gst-omx video decoder base class");

G_DEFINE_TYPE_WITH_CODE (GstOMXRVDec, gst_omx_rv_dec,
    GST_TYPE_OMX_VIDEO_DEC, DEBUG_INIT);


static void
gst_omx_rv_dec_class_init (GstOMXRVDecClass * klass)
{
  GstElementClass *element_class = GST_ELEMENT_CLASS (klass);
  GstOMXVideoDecClass *videodec_class = GST_OMX_VIDEO_DEC_CLASS (klass);

  videodec_class->is_format_change =
      GST_DEBUG_FUNCPTR (gst_omx_rv_dec_is_format_change);
  videodec_class->set_format = GST_DEBUG_FUNCPTR (gst_omx_rv_dec_set_format);

  videodec_class->cdata.default_sink_template_caps = "video/x-pn-realvideo, "
      "width=(int) [1,MAX], " "height=(int) [1,MAX]";

  gst_element_class_set_static_metadata (element_class,
      "OpenMAX RV Video Decoder",
      "Codec/Decoder/Video", "Decode RV video streams", "MStar");

  gst_omx_set_default_role (&videodec_class->cdata, "video_decoder.rv");
}

static void
gst_omx_rv_dec_init (GstOMXRVDec * self)
{
}

static gboolean
gst_omx_rv_dec_is_format_change (GstOMXVideoDec * dec,
    GstOMXPort * port, GstVideoCodecState * state)
{
  return FALSE;
}
...
static gboolean
gst_omx_rv_dec_set_format (GstOMXVideoDec * dec, GstOMXPort * port,
