...
gst_omx_analog_audio_sink_class_init (GstOMXAnalogAudioSinkClass * klass)
{
  ...
  audiosink_class->cdata.default_sink_template_caps = "audio/x-raw, "
      ...
      "rate = (int) [ 8000, 96000 ], " "channels = (int) [ 1, 12 ] ";
...
{
  gst_base_sink_set_max_lateness (GST_BASE_SINK (self), -1 /*360000000000 */ );
//    gst_base_sink_set_sync(GST_BASE_SINK (self), FALSE);    // set freerun // noted by kochien.kuo
}
