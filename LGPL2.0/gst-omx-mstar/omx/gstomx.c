...
gst_omx_core_acquire (const gchar * filename)
{
  ...
  if (!core) {
    ...
    if (!g_module_symbol (core->module, "MS_OMX_Init",
            (gpointer *) & core->init))
      goto symbol_error;
    if (!g_module_symbol (core->module, "MS_OMX_DeInit",
            (gpointer *) & core->deinit))
      goto symbol_error;
    if (!g_module_symbol (core->module, "MS_OMX_GetHandle",
            (gpointer *) & core->get_handle))
      goto symbol_error;
    if (!g_module_symbol (core->module, "MS_OMX_FreeHandle",
            (gpointer *) & core->free_handle))
      goto symbol_error;
    if (!g_module_symbol (core->module, "MS_OMX_SetupTunnel",
    ...
    gboolean flush)
{
  ...
  if (flush) {
    ...
    gst_omx_component_handle_messages (comp);
    /*Discuss with Ted Team and Howard to remove buffer owned checking.
       Becasue we should not skip "port->flushed" checking even buffers are all owned. */
#if 0
    ...
    if (port->buffers
        && port->buffers->len > g_queue_get_length (&port->pending_buffers))
      GST_ERROR_OBJECT (comp->parent,
          "Buffers are not all owned by gst-omx!!!");
...
gst_omx_port_wait_enabled_unlocked (GstOMXPort * port, GstClockTime timeout)
{
  ...
  gst_omx_port_update_port_definition (port, NULL);
  gst_omx_component_handle_messages (comp);
  /* Add below line to prevent IL port is updated just between gst_omx_port_update_port_definition and gst_omx_component_handle_messages.
     If IL port is really updated between them. We don't have chance to jump out of below loop except for timeout. Discuss with GST video/audio team. */
  gst_omx_port_update_port_definition (port, NULL);
...
static const GGetTypeFunction types[] = {
  ...
  gst_omx_h264_dec_get_type, gst_omx_mvc_dec_get_type,
  gst_omx_h263_dec_get_type,
  gst_omx_divx4_dec_get_type, gst_omx_divx3_dec_get_type,
  gst_omx_rv_dec_get_type,
  gst_omx_vp8_dec_get_type, gst_omx_vp9_dec_get_type, gst_omx_avs_dec_get_type,
  gst_omx_hevc_dec_get_type, gst_omx_vc1_dec_get_type, gst_omx_flv_dec_get_type,
  ...
  gst_omx_mp3_dec_get_type, gst_omx_mp2_dec_get_type,
  gst_omx_wma_dec_get_type, gst_omx_ac3_dec_get_type,
  gst_omx_aac_dec_get_type, gst_omx_vorbis_dec_get_type,
  gst_omx_ra_dec_get_type, gst_omx_dts_dec_get_type,
  gst_omx_adpcm_dec_get_type, gst_omx_amr_dec_get_type,
  gst_omx_flac_dec_get_type,
  gst_omx_lpcm_dec_get_type
#ifdef HAVE_THEORA
...
gst_omx_parse_hacks (gchar ** hacks)
{
  ...
  while (*hacks) {
    ...
    else if (g_str_equal (*hacks, "share-outport-buffer"))
      hacks_flags |= GST_OMX_HACK_SHARE_OUTPORT_BUFFER;
    else
