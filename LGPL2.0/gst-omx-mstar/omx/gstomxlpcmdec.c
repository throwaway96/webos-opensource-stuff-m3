...
#include "gstomxlpcmdec.h"

GST_DEBUG_CATEGORY_STATIC (gst_omx_lpcm_dec_debug_category);
#define GST_CAT_DEFAULT gst_omx_lpcm_dec_debug_category

/* prototypes */
static gboolean gst_omx_lpcm_dec_set_format (GstOMXAudioDec * dec,
    GstOMXPort * port, GstCaps * caps);
static gboolean gst_omx_lpcm_dec_is_format_change (GstOMXAudioDec * dec,
    GstOMXPort * port, GstCaps * caps);
static gint gst_omx_lpcm_dec_get_samples_per_frame (GstOMXAudioDec * dec,
    GstOMXPort * port);

/* class initialization */

#define DEBUG_INIT \
  GST_DEBUG_CATEGORY_INIT (gst_omx_lpcm_dec_debug_category, "omxlpcmdec", 0, \
      "debug category for gst-omx audio decoder base class");

G_DEFINE_TYPE_WITH_CODE (GstOMXLPCMDec, gst_omx_lpcm_dec,
    GST_TYPE_OMX_AUDIO_DEC, DEBUG_INIT);


static void
gst_omx_lpcm_dec_class_init (GstOMXLPCMDecClass * klass)
{
  GstElementClass *element_class = GST_ELEMENT_CLASS (klass);
  GstOMXAudioDecClass *audiodec_class = GST_OMX_AUDIO_DEC_CLASS (klass);

  audiodec_class->set_format = GST_DEBUG_FUNCPTR (gst_omx_lpcm_dec_set_format);
  audiodec_class->is_format_change =
      GST_DEBUG_FUNCPTR (gst_omx_lpcm_dec_is_format_change);
  audiodec_class->get_samples_per_frame =
      GST_DEBUG_FUNCPTR (gst_omx_lpcm_dec_get_samples_per_frame);
  ...
  gst_element_class_set_static_metadata (element_class,
      "OpenMAX LPCM Audio Decoder",
      "Codec/Decoder/Audio",
      "Decode LPCM audio streams", "MStar Semiconductor Inc");

  gst_omx_set_default_role (&audiodec_class->cdata, "audio_decoder.lpcm");
}

static void
gst_omx_lpcm_dec_init (GstOMXLPCMDec * self)
{
  self->spf = -1;
}

static gboolean
gst_omx_lpcm_dec_set_format (GstOMXAudioDec * dec, GstOMXPort * port,
    GstCaps * caps)
{
  GstOMXLPCMDec *self = GST_OMX_LPCM_DEC (dec);
  OMX_PARAM_PORTDEFINITIONTYPE port_def;
  OMX_ERRORTYPE err;
#if 0                           // to fix compile warning
  OMX_AUDIO_PARAM_LPCMTYPE lpcm_param;
  ...
  GST_DEBUG_OBJECT (caps, "LPCM Caps");
  //  get port def
  ...
  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (self,
        "Failed to set LPCM format on component: %s (0x%08x)",
  ...
  if (g_str_equal (dec->app_type, "RTC")) {
    /* FIXME: patch for LPCM can not connect miracast */
    /* the miracast mimetype is audio/x-private1-lpcm */
    /* but need to set "audio_decoder.lpcm", need to clarify */
    g_stpcpy ((gchar *) ComponentRole.cRole, "audio_decoder.lpcm");
    ret =
        gst_omx_component_set_parameter (dec->dec,
        OMX_IndexParamStandardComponentRole, &ComponentRole);
  } else {
    if (g_str_equal (mimetype, "audio/x-private1-lpcm")) {
      g_stpcpy ((gchar *) ComponentRole.cRole, "audio_decoder.lpcm_1");
      ret =
          gst_omx_component_set_parameter (dec->dec,
          OMX_IndexParamStandardComponentRole, &ComponentRole);
    } else {
...
static gboolean
gst_omx_lpcm_dec_is_format_change (GstOMXAudioDec * dec, GstOMXPort * port,
    GstCaps * caps)
{
  return FALSE;
}
...
static gint
gst_omx_lpcm_dec_get_samples_per_frame (GstOMXAudioDec * dec, GstOMXPort * port)
{
  ...
  if (dec->rate == 0.5) {
    ...
    return GST_OMX_LPCM_DEC (dec)->spf;
  }
}
