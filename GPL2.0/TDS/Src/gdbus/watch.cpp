                            ...
                            const char *argument)
{
    ...
    for (current = listeners;
        ...
        struct filter_data *data = (struct filter_data *)current->data;
                    ...
                    DBusMessage *message, void *user_data)
{
    struct filter_data *data = (struct filter_data *) user_data;
    struct filter_callback *cb;
    char *_name, *_old, *_new;

    if (!dbus_message_get_args(message, NULL,
                DBUS_TYPE_STRING, &_name,
                DBUS_TYPE_STRING, &_old,
                DBUS_TYPE_STRING, &_new,
                DBUS_TYPE_INVALID)) {
        error("Invalid arguments for NameOwnerChanged signal");
        return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
    }

    update_name_cache(_name, _new);
...
gboolean g_dbus_remove_watch(DBusConnection *connection, guint id)
{
    ...
    for (ldata = listeners; ldata; ldata = ldata->next) {
        data = (struct filter_data *)ldata->data;
