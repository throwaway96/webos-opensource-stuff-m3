                ...
                struct generic_data *data, const char *path)
{
    ...
    for (list = data->interfaces; list; list = list->next) {
        struct interface_data *iface = (struct interface_data *)list->data;
...
{
    struct generic_data *data = (struct generic_data *)user_data;
    DBusMessage *reply;
                    ...
                    GDBusPendingReply pending)
{
    GSList *list;
        ...
        struct security_data *secdata = (struct security_data *)list->data;
...
{
    struct builtin_security_data *data = (struct builtin_security_data *)user_data;
                        ...
                        GDBusPendingReply pending)
{
    ...
    data = (struct builtin_security_data *) g_new0(struct builtin_security_data, 1);
    data->conn = conn;
            ...
            const GDBusMethodTable *method, void *iface_user_data)
{
    ...
    for (security = security_table; security && security->privilege;
        ...
        secdata = (struct security_data *) g_new(struct security_data, 1);
                        ...
                        GDBusPendingPropertySet id)
{
    ...
    for (l = pending_property_set; l != NULL; l = l->next) {
        propdata = (struct property_data *) l->data;
        if (propdata->id != id)
            continue;
...
{
    struct generic_data *child = (struct generic_data *)data;
    struct generic_data *parent = (struct generic_data *)user_data;
...
{
    struct interface_data *iface = (struct interface_data *) data;
...
{
    struct generic_data *data = (struct generic_data *) user_data;
    struct generic_data *parent = (struct generic_data *) data->parent;
                            ...
                            const char *path)
{
    ...
    generic_table.unregister_function    = generic_unregister;
    generic_table.message_function        = generic_message;
    ...
    static GDBusMethodTable introspect_methods[2];
    memset(&introspect_methods[0], 0, sizeof(introspect_methods));

    introspect_methods[0].name = "Introspect";
    introspect_methods[0].in_args = NULL;
    introspect_methods[0].out_args = GDBUS_ARGS({ "xml", "s" });
    introspect_methods[0].function = introspect;
                    ...
                    GDBusDestroyFunction destroy)
{
    ...
    static GDBusMethodTable properties_methods[4];
    memset(&properties_methods[0], 0, sizeof(properties_methods));

    properties_methods[0].name = "Get";
    properties_methods[0].in_args = GDBUS_ARGS({ "interface", "s" }, { "name", "s" });
    properties_methods[0].out_args = GDBUS_ARGS({ "value", "v" });
    properties_methods[0].function = properties_get;

    properties_methods[1].name = "Set";
    properties_methods[1].in_args = GDBUS_ARGS({ "interface", "s" }, { "name", "s" }, { "value", "v" });
    properties_methods[1].out_args = NULL;
    properties_methods[1].function = properties_set;

    properties_methods[2].name = "GetAll";
    properties_methods[2].in_args = GDBUS_ARGS({ "interface", "s" });
    properties_methods[2].out_args = GDBUS_ARGS({ "properties", "a{sv}" });
    properties_methods[2].function = properties_get_all;

    static GDBusSignalTable properties_signals[2];
    memset(&properties_methods[0], 0, sizeof(properties_methods));

    properties_signals[0].name = "PropertiesChanged";
    properties_signals[0].args = GDBUS_ARGS({ "interface", "s" }, { "changed_properties", "a{sv}" }, { "invalidated_properties", "as"});
...
static void process_property_changes(struct generic_data *data)
{
    ...
    for (l = data->interfaces; l != NULL; l = l->next) {
        struct interface_data *iface = (struct interface_data *) l->data;
...
gboolean g_dbus_attach_object_manager(DBusConnection *connection)
{
    ...
    manager_methods[0].name = "GetManagedObjects";
    manager_methods[0].in_args = NULL;
    manager_methods[0].out_args = GDBUS_ARGS({ "objects", "a{oa{sa{sv}}}" });
    manager_methods[0].function = get_objects;

    static GDBusSignalTable manager_signals[3];
    memset(&manager_signals[0], 0, sizeof(manager_signals));

    manager_signals[0].name = "InterfacesAdded";
    manager_signals[0].args = GDBUS_ARGS({ "object", "o" }, { "interfaces", "a{sa{sv}}" });

    manager_signals[1].name = "InterfacesRemoved";
    manager_signals[1].args = GDBUS_ARGS({ "object", "o" }, { "interfaces", "as" });
