...
sPppdInfo g_pppInfo = {0,0,NULL,NULL,NULL,{NULL,},NULL,0,TDS_ERROR_TYPE_NO_ERROR,NULL,0,false,false,false};
bool g_pppSyncVar;
void *g_callbackData = NULL;
...
{
	sPppdInfo *sPPPData = (sPppdInfo *)data;

    struct pri_context *ctx = (struct pri_context *) sPPPData->tdsData;
    DBusConnection *conn = tds_dbus_get_connection();
    tds_bool_t value;
	...
	if(sPPPData->bPPPStatus == TRUE)
	{
		 ctx->settings->interface = g_strdup((const char *)sPPPData->interface);

		 TDS_LOG_DEBUG("ctx->settings->interface = [%s]", ctx->settings->interface);

		ctx->active = TRUE;
	    	ctx->settings->ipv4->static_ip = TRUE;
		ctx->settings->ipv4->ip = sPPPData->ipv4->ip;
		ctx->settings->ipv4->netmask = sPPPData->ipv4->netmask;
		ctx->settings->ipv4->gateway = sPPPData->ipv4->gateway;

		char *dns[3] = {NULL,};

		if(ctx->settings->ipv4->dns)
			g_strfreev(ctx->settings->ipv4->dns);
		dns[0] = sPPPData->ipv4->dns[0];
		dns[1] = sPPPData->ipv4->dns[1];
	...
	sPPPData->bPdpActivateInProgress = false;
...
void til_handle_Activate(bool ret, void *data)
{
	...
	if(g_pppInfo.procID > 0)
	{
		...
		pri_activate_callback(error, &g_pppInfo);

		return;
	}

	g_pppInfo.bPPPStatus = false;
	g_pppInfo.interface = NULL;
	g_pppInfo.ipv4 = NULL;
	g_pppInfo.portOfComm = NULL;
	g_pppInfo.speed = NULL;
	g_pppInfo.ipv4 = NULL;

	g_pppInfo.tdsData = ctx;
	g_pppInfo.tilErr  = error;

	if(ctx->active == TRUE)
	{
		if(g_pppInfo.ipv4 == NULL)
		{
			g_pppInfo.ipv4 = (struct ipv4_settings *)g_try_malloc (sizeof(struct ipv4_settings));

		if(g_pppInfo.ipv4 != NULL)
		{
			g_pppInfo.ipv4->dns = (char **)g_try_malloc (sizeof(char *)*3);
			g_pppInfo.ipv4->dns[0] = NULL;
			g_pppInfo.ipv4->dns[1] = NULL;
			g_pppInfo.ipv4->dns[2] = NULL;
			g_pppInfo.ipv4->gateway = NULL;
			g_pppInfo.ipv4->ip = NULL;
			g_pppInfo.ipv4->netmask = NULL;
		}
		else
			ctx->active = FALSE;
	}

		g_pppInfo.portOfComm = "ttyUSB0";
		g_pppInfo.speed = "115200";

	    __tds_dbus_pending_reply(&ctx->pending,
                dbus_message_new_method_return(ctx->pending));

		cb_execute(&g_pppInfo);
	}
	else
	{
...
void til_handle_ConnStatus(string &ConnState, void *data)
{
    ...
    else if(ConnState.compare("disconnected") == 0 || ConnState.compare("no carrier") == 0 )
	{
        ctx->active = FALSE;
    }
	else
...
void til_sends_fake_apn(void *data,char *i_strAPN)
{
	if(i_strAPN != NULL)
	{
		...
		TDS_LOG_DEBUG("AccessPoint [%s] path [%s]",i_strAPN,ctx->path);

		strcpy(ctx->context.apn, i_strAPN);

		tds_dbus_signal_property_changed(conn, ctx->path,
						TDS_CONNECTION_CONTEXT_INTERFACE,
						"AccessPointName",
						DBUS_TYPE_STRING, &i_strAPN);
	}
}
                    ...
                    DBusMessage *msg, void *data)
{
    ...
    if (g_str_equal(property, "Active")) {
		...
		if((ctx->context.apn == NULL) || (strcmp(ctx->context.apn,DEFAULT_APN) == 0))
		{
			TDS_LOG_DEBUG("Either apn is not set or invalid");
...
static void release_active_contexts(struct tds_gprs *gprs)
{
    ...
    for (l = gprs->contexts; l; l = l->next) {
        ...
        // deactivate context itself
        // send deactivate messages and update ctx->active = false
...
cb_child_watch( GPid  pid,
                ...
                gpointer *i_pData )
{
	sPppdInfo *sData = (sPppdInfo *)i_pData;
	tds_bool_t value = false;
	struct pri_context *ctx = (struct pri_context *) sData->tdsData;


	TDS_LOG_DEBUG("PPPD daemon is killed");

	if(sData->pppWatchSrcID != 0)
	{
		g_source_remove(sData->pppWatchSrcID);
		sData->pppWatchSrcID = 0;
	}

	g_spawn_close_pid( pid);

	sData->ipInfoRetryCount = 3;
	sData->procID = 0;

	if(sData->bPPPKilling == false && !sData->bPdpActivateInProgress)
	{
		DBusConnection *conn = tds_dbus_get_connection();

		if(conn == NULL)
		{
			TDS_LOG_DEBUG("cb_child_watch:: No Connman Connection");
		}
		ctx->active = false;

		value = ctx->active;

		TDS_LOG_DEBUG("cb_child_watch:: ctx->active will be update as [%s]", value);
...
cb_execute(sPppdInfo *i_pData)
{
	...
	argv[1] = i_pData->portOfComm;
	argv[2] = i_pData->speed;
	argv[3] = "usepeerdns";
	argv[4] = "nodetach";
	argv[5] = "crtscts";
	argv[6] = "noauth";
	...
	{
		if(g_remove("/var/run/resolv.conf") != 0)
		{
			TDS_LOG_DEBUG("Removed /var/run/resolv.conf");
		}
	}

	if (g_file_test ("/var/run/pppinterface", G_FILE_TEST_EXISTS))
	{
		if(g_remove("/var/run/pppinterface") != 0)
		{
			TDS_LOG_DEBUG("Removed /var/run/pppinterface");
		}
	}

	if (g_file_test ("/var/run/pppiplocal", G_FILE_TEST_EXISTS))
	{
		if(g_remove("/var/run/pppiplocal") != 0)
		{
			TDS_LOG_DEBUG("Removed /var/run/pppiplocal");
		}
	}

	if (g_file_test ("/var/run/pppipremote", G_FILE_TEST_EXISTS))
	{
		if(g_remove("/var/run/pppipremote") != 0)
		{
			TDS_LOG_DEBUG("Removed /var/run/pppipremote");
		}
	}

	if (g_file_test ("/var/run/ppptty", G_FILE_TEST_EXISTS))
	{
		if(g_remove("/var/run/ppptty") != 0)
		{
			TDS_LOG_DEBUG("Removed /var/run/ppptty");
		}
	}

	i_pData->bPPPKilling = false;
	i_pData->bPdpActivateInProgress = true;
    ret = g_spawn_async_with_pipes( NULL, argv, NULL,
                                    G_SPAWN_DO_NOT_REAP_CHILD, NULL,
                                    NULL, &(i_pData->procID), NULL, NULL, NULL, NULL );
    if( ! ret )
    {
        TDS_LOG_DEBUG("PPPD SPAWN FAILED");
        return;
    }
	else
	{
		TDS_LOG_DEBUG("pppd proc ID :: 0x%x",i_pData->procID);
	}

    i_pData->pppWatchSrcID = g_child_watch_add( i_pData->procID, (GChildWatchFunc)cb_child_watch, i_pData );

	i_pData->ipInfoRetryCount = 0;
	i_pData->timeout_id = g_timeout_add( 10000, (GSourceFunc)cb_timeout, i_pData );
...
cb_timeout( gpointer *i_pData )
{
	...
	sPppdInfo *pData = (sPppdInfo *)i_pData;

	if(pData->procID == 0)
	{
		TDS_LOG_DEBUG("cb_timeout:: pppd is already killed");

		return FALSE;
	}

	dns1 = (char *)g_try_malloc0 (32);
	dns2 = (char *)g_try_malloc0 (32);
	ipAddress = (char *)g_try_malloc0 (32);
	interfaceName = (char *)g_try_malloc0 (32);
	gateway = (char *)g_try_malloc0 (32);

	TDS_LOG_DEBUG("");

	if(!dns1 || !dns2 || !ipAddress || !interfaceName || !gateway)
	{
		nErr = -1;
		goto END;
	}
	...
	if (g_file_test ("/var/run/resolv.conf", G_FILE_TEST_EXISTS))
	{
		pFile = fopen("/var/run/resolv.conf","r");
		...
		TDS_LOG_DEBUG("File opened :: [/var/run/resolv.conf]");
		while(!feof(pFile))
		{
			if(nString == 1)
			{
			...
			else if(nString == 3)
			{
				...
				fscanf(pFile,"%s",tmpStr);

			nString++;
		}

		TDS_LOG_DEBUG("[/var/run/resolv.conf] DNS1 [%s] DNS2 [%s]",dns1,dns2);
		fclose(pFile);
		pFile = NULL;
	...
	if (g_file_test ("/var/run/pppinterface", G_FILE_TEST_EXISTS))
	{
		pFile = fopen("/var/run/pppinterface","r");
		...
		TDS_LOG_DEBUG("File opened :: [/var/run/pppinterface]");

		while(!feof(pFile))
		{
			fscanf(pFile,"%s",interfaceName);
		}

		TDS_LOG_DEBUG("[/var/run/pppinterface] interface [%s]",interfaceName);
	...
	if (g_file_test ("/var/run/pppiplocal", G_FILE_TEST_EXISTS))
	{
		pFile = fopen("/var/run/pppiplocal","r");
		...
		TDS_LOG_DEBUG("[/var/run/ppppiplocal] [%s]",ipAddress);
	...
	if (g_file_test ("/var/run/pppipremote", G_FILE_TEST_EXISTS))
	{
		pFile = fopen("/var/run/pppipremote","r");
		...
		while(!feof(pFile))
		{
			fscanf(pFile,"%s",gateway);
		}

		TDS_LOG_DEBUG("[/var/run/pppipremote] [%s]",gateway);
...
END:
	...
	TDS_LOG_DEBUG("PPP Error Status [%d]",nErr);

	if(nErr == -1)
	{
		pData->bPPPStatus = false;
		pData->tilErr.type = TDS_ERROR_TYPE_FAILURE;

		if(pData->procID != 0)
		{
			pData->bPPPKilling = true;
			if( kill(pData->procID, SIGNAL_KILL) == -1)
			{
				TDS_LOG_DEBUG("Failed to kill pppd Daemon");
			}
			else
			{
				TDS_LOG_DEBUG("PPPDaemon Killed");
			}
		}

		if(!dns1 || !dns2 || !ipAddress || !interfaceName || !gateway)
		{
			TDS_LOG_DEBUG("Memory allocation failed");
	...
	{
		TDS_LOG_DEBUG("cb_timeout:: pppd Execute success");

		pData->bPPPStatus = true;

		TDS_LOG_DEBUG("cb_timeout:: bPPPStatus set to True");

		if(pData->ipv4 == NULL)
			TDS_LOG_DEBUG("cb_timeout:: ipv4 is NULL");
		else
			TDS_LOG_DEBUG("cb_timeout:: ipv4 is Not NULL");

		if(NULL != pData->ipv4->dns[0])
		{
			TDS_LOG_DEBUG("cb_timeout:: ipv4->dns[0] != NULL ");
			g_free (pData->ipv4->dns[0]);
			pData->ipv4->dns[0] = NULL;
			TDS_LOG_DEBUG("");
		}

		if(NULL != pData->ipv4->dns[1])
		{
			TDS_LOG_DEBUG("cb_timeout:: ipv4->dns[1] != NULL ");
			g_free (pData->ipv4->dns[1]);
			pData->ipv4->dns[1] = NULL;
		}

		if(NULL != pData->ipv4->gateway)
		{
			TDS_LOG_DEBUG("cb_timeout:: ipv4->gateway != NULL ");
			g_free (pData->ipv4->gateway);
			pData->ipv4->gateway = NULL;
		}

		if(NULL != pData->ipv4->ip)
		{
			TDS_LOG_DEBUG("cb_timeout:: ipv4->ip != NULL ");
			g_free (pData->ipv4->ip);
			pData->ipv4->ip = NULL;
		}


		TDS_LOG_DEBUG("cb_timeout:: Assign latest ip information");
		pData->ipv4->ip = ipAddress;
		pData->ipv4->gateway = gateway;

		if(strlen(dns1) != 0)
			pData->ipv4->dns[0] = dns1;
		if(strlen(dns2) != 0)
			pData->ipv4->dns[1] = dns2;
		pData->interface = interfaceName;

	}

	pri_activate_callback(pData->tilErr,pData);
